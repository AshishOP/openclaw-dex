...
const SEARCH_PROVIDERS = ["brave", "perplexity", "ddgs"] as const;
...
export function createWebSearchTool(options?: {
  ...
  // DEX EDITION: Check if DDGS (free search) is configured
  if (isDDGSProvider(options?.config)) {
    const ddgsConfig = createDDGSSearchConfig(options?.config);
    return {
      label: "Web Search",
      name: "web_search",
      description: "Search the web using DuckDuckGo (DDGS). FREE - no API key required!",
      parameters: WebSearchSchema,
      execute: async (_toolCallId, args) => {
        const params = args as Record<string, unknown>;
        const query = readStringParam(params, "query", { required: true });
        const count = readNumberParam(params, "count", { integer: true }) ?? ddgsConfig.maxResults;
        const result = await runDDGSSearch({
          query,
          count: Math.min(count, MAX_SEARCH_COUNT),
          timeoutSeconds: ddgsConfig.timeoutSeconds,
