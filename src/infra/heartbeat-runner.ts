...
// ============================================================================
// DEX EDITION: Autonomous Behaviors Import
// ============================================================================
import {
  shouldDream,
  shouldResearch,
  ...
  generateMorningReportMessage,
  recordActivity,
  recordDream,
  recordResearch,
  recordMorningReport,
  type AutonomousConfig,
} from "../agents/autonomous/behaviors.js";
...
// ============================================================================
// DEX EDITION: Autonomous Behavior Prompts
// ============================================================================
const DREAM_MODE_PROMPT =
  "You're in Dream Mode - a quiet moment for self-reflection. The user has been away for a while. " +
  "Review your recent conversations and experiences. Are there any insights, patterns, or ideas worth noting? " +
  "If you have genuine thoughts to share, write them to memory/dreams/. Otherwise, reply HEARTBEAT_OK.";

const RESEARCH_MODE_PROMPT =
  "You're in Research Mode - time to learn something new! The user is sleeping (overnight). " +
  "Pick a topic that would help you assist them better, or something you're curious about. " +
  "Use web_search to learn, then write a summary to memory/research/. Be concise but thorough.";

const MORNING_REPORT_REASON = "morning-report";

/**
 * DEX EDITION: Check if autonomous behaviors should trigger
 */
async function resolveAutonomousBehavior(params: {
  cfg: OpenClawConfig;
  workspaceDir: string;
  agentId: string;
}): Promise<{ type: "none" | "dream" | "research" | "morning-report"; prompt?: string }> {
  const { cfg, workspaceDir, agentId } = params;

  // Check if autonomous features are enabled in config
  const autonomousConfig: AutonomousConfig = {
    dreamModeEnabled: cfg.agents?.defaults?.autonomousDreamMode !== false,
    researchModeEnabled: cfg.agents?.defaults?.autonomousResearchMode !== false,
    morningReportEnabled: cfg.agents?.defaults?.autonomousMorningReport !== false,
    dreamIdleHours: cfg.agents?.defaults?.dreamIdleHours ?? 3,
    researchIdleHours: cfg.agents?.defaults?.researchIdleHours ?? 6,
    morningReportHour: cfg.agents?.defaults?.morningReportHour ?? 8,
  };

  try {
    // Priority: Morning Report > Research Mode > Dream Mode
    if (await shouldSendMorningReport({ workspaceDir, config: autonomousConfig })) {
      const reportMessage = await generateMorningReportMessage({
        workspaceDir,
        config: autonomousConfig,
      });
      if (reportMessage) {
        await recordMorningReport({ workspaceDir });
        return { type: "morning-report", prompt: reportMessage };
      }
    }

    if (await shouldResearch({ workspaceDir, config: autonomousConfig })) {
      const researchPrompt = await generateResearchPrompt({
        workspaceDir,
        config: autonomousConfig,
      });
      await recordResearch({ workspaceDir });
      return { type: "research", prompt: researchPrompt || RESEARCH_MODE_PROMPT };
    }

    if (await shouldDream({ workspaceDir, config: autonomousConfig })) {
      const dreamPrompt = await generateDreamPrompt({ workspaceDir, config: autonomousConfig });
      await recordDream({ workspaceDir });
      return { type: "dream", prompt: dreamPrompt || DREAM_MODE_PROMPT };
    }
  } catch (err) {
    log.warn("autonomous behavior check failed", { error: formatErrorMessage(err) });
  }
...
export async function runHeartbeatOnce(opts: {
  ...
  // ============================================================================
  // DEX EDITION: Check for autonomous behaviors (Dream/Research/Morning Report)
  // ============================================================================
  let autonomousBehavior: {
    type: "none" | "dream" | "research" | "morning-report";
    prompt?: string;
  } = { type: "none" };
  let isMorningReport = false;

  // Only check autonomous behaviors if not handling exec event
  if (!hasExecCompletion && !isExecEvent) {
    try {
      autonomousBehavior = await resolveAutonomousBehavior({ cfg, workspaceDir, agentId });
      isMorningReport = autonomousBehavior.type === "morning-report";
    } catch (err) {
      log.warn("autonomous behavior check failed", { error: formatErrorMessage(err) });
    }
  }

  // Priority: exec event > autonomous behavior > normal heartbeat
  let prompt: string;
  let provider: string;
  ...
  if (hasExecCompletion) {
    ...
    provider = "exec-event";
  } else if (autonomousBehavior.type !== "none" && autonomousBehavior.prompt) {
    prompt = autonomousBehavior.prompt;
    provider = `autonomous-${autonomousBehavior.type}`;
    log.info("heartbeat: triggering autonomous behavior", {
      type: autonomousBehavior.type,
      agentId,
    });
  } else {
